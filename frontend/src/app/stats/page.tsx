    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {stats ? (
            ...
            <div className="bg-white rounded-xl shadow-md p-6 mb-8 border border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-sm text-gray-500 mb-1">Most Recent Job</p>
                  <p className="text-gray-900 font-medium">{formatDistanceToNow(overall.newestJob)}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-sm text-gray-500 mb-1">Oldest Job</p>
                  <p className="text-gray-900 font-medium">{formatDistanceToNow(overall.oldestJob)}</p>
                </div>
              </div>
            </div>
          )}
                    ...
                    return (
                      <div key={index}>
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-sm font-medium text-gray-700">{loc._id}</span>
                          <span className="text-sm text-gray-500">{loc.count} jobs ({percentage.toFixed(0)}%)</span>
                        </div>
                    ...
                    return (
                      <div key={index} className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-sm font-medium text-gray-900 capitalize">{src._id.replace('-', ' ')}</span>
                            <span className="text-sm font-semibold text-purple-600">{src.count}</span>
                          </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 mt-6 border border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                {topSkills.slice(0, 20).map((skill, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-emerald-50 to-teal-50 text-emerald-700 rounded-full text-sm font-medium border border-emerald-100"
                  >
                    {skill._id}
                    <span className="bg-emerald-100 text-emerald-600 px-1.5 py-0.5 rounded-full text-xs">
                      {skill.count}
                    </span>
                  </span>
                ))}
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-sm mt-1">Add some jobs to see analytics</p>
        </div>
      )}
    </div>
  );
